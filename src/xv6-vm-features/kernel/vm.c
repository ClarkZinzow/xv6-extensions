...
copyuvm(pde_t *pgdir, uint szcd)
{
  ...
  hpbase = PGROUNDUP(szcd) + PGSIZE;
  sz = proc->sz;

  for(i = hpbase; i < sz; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
  ...
  for(i = PGSIZE; i < stsz; i += PGSIZE){
    stva = (USERTOP - stsz + i);
    if((pte = walkpgdir(pgdir, (void*)stva, 0)) == 0)
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
    memmove(mem, (char*)pa, PGSIZE);
    if(mappages(d, (void*)stva, PGSIZE, PADDR(mem), PTE_W|PTE_U) < 0)
      goto bad;
  }
  return d;
