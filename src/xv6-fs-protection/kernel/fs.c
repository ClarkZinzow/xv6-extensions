...
stati(struct inode *ip, struct stat *st)
{
  ...
  st->physical_size = (ip->type == T_MIRRORED) ? 3*ip->size : ip->size;
}
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    if(ip->type == T_MIRRORED){
      ...
      uint mirror = bmap(ip, off/BSIZE, 1);
      uint reserve = bmap(ip, off/BSIZE, 2);
      
      int comp[3] = {0, 0, 0};
      
      // read the default file block
      bp = bread(ip->dev, addr);
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    if(ip->type == T_MIRRORED){
      uint addr =  bmap(ip, off/BSIZE, 0);
      uint mAddr = bmap(ip, off/BSIZE, 1);
      uint rAddr = bmap(ip, off/BSIZE, 2);
